		...
		case ARG_flash:
			...
			if (strcmp(argv[0], "enter") == 0) {
				...
				if (argc == 2) {
					...
					ret = bootloader_crc(fp, 0, getNumFlashSectors(), false);
					
				} else if (argc == 4 || verbose) {
					u32 first_sector = atol(argv[2]);
					if (first_sector >= getNumFlashSectors()) first_sector = getNumFlashSectors() - 1;
					u32 num_sectors = atol(argv[3]);
					if (num_sectors == 0) num_sectors = 1;	// Doesn't make sense to do zero.
					if (first_sector + num_sectors > getNumFlashSectors()) {
						num_sectors = getNumFlashSectors() - first_sector;
					}
					fp = fopen(SANITIZE(argv[1]), "rb");
					if (!fp)
					{
